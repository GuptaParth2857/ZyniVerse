...
function formatYear(year: number, endYear?: number) {
  const start = year > 0 ? String(year) : "—";
  return endYear ? `${start}–${endYear}` : start;
}
    ...
    <Link href={`/live-action/${anime.id}`} className="shrink-0 w-[130px] sm:w-[150px] group/card">
      <div className="relative aspect-[2/3] overflow-hidden rounded-xl neon-rgb-border bg-[var(--color-panel)] transition-all duration-300 group-hover/card:border-[var(--color-magenta)] group-hover/card:shadow-[0_0_30px_-8px_var(--color-magenta)]">
        {anime.posterUrl && !imgErr ? (
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-[var(--color-cyan)]/20 to-[var(--color-magenta)]/20 flex items-center justify-center p-2 text-center">
...
export default function LiveActionDetailClient({ id, initialList }: { id: string; initialList: LiveActionAnime[] }) {
  ...
  const sameSource = useMemo(() => {
    if (!anime) return [];
    return initialList.filter((a) => a.id !== anime.id && a.basedOn === anime.basedOn);
  }, [anime, initialList]);
        ...
        <div>
              ...
              <div className="flex flex-col gap-6 sm:flex-row sm:items-end">
                ...
                <div className="min-w-0 flex-1 pb-4 text-center sm:text-left">
                  ...
                  <div className="mt-3 flex flex-wrap items-center justify-center sm:justify-start gap-3 text-sm text-[var(--color-mute)]">
                    ...
                    <span>{formatYear(anime.releaseYear, anime.endYear)}</span>
                  </div>
          ...
          <div className="border-b border-[var(--color-line)] bg-[var(--color-panel)]">
            <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-x-8 gap-y-2 px-4 py-3 text-sm sm:px-6">
              ...
              <span className="font-mono text-xs text-[var(--color-mute)]">
                {formatYear(anime.releaseYear, anime.endYear)}
              </span>
              ...
              <div className="rounded-xl neon-rgb-border bg-[var(--color-panel)] p-5">
                ...
                <div className="space-y-3 text-sm">
                  {[
                    ...
                    { label: "Release", value: formatYear(anime.releaseYear, anime.endYear) },
              ...
              <div className="rounded-xl neon-rgb-border bg-[var(--color-panel)] p-5">
                ...
                <div className="space-y-2">
                  {anime.platforms.length === 0 ? (
                    <p className="text-xs text-[var(--color-mute)]">Streaming platform not announced yet</p>
                  ) : (
                    anime.platforms.map((p) => (
                      <div
                        ...
                        style={{
                          borderColor: `${p.logoColor}44`,
                          background: `${p.logoColor}08`,
                        }}
                      >
                        <div
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {p.subtitle && <p className="text-[10px] text-[var(--color-mute)] truncate">{p.subtitle}</p>}
                          {!p.available && (
                            <p className="text-[10px] font-semibold text-[var(--color-amber)]">Coming Soon</p>
                          )}
                        </div>
                        {p.available && (
