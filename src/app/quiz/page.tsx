  ...
  return (
    <div className="min-h-screen animate-page-in">
      ...
      <div className="relative overflow-hidden border-b border-[var(--color-line)]">
        ...
        <div className="relative mx-auto max-w-5xl px-4 py-14 sm:px-6 sm:py-20">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            ...
            <Link href="/quiz/play" className="neon-rgb-border rounded-xl bg-[var(--color-panel)]/60 backdrop-blur-sm px-6 py-3 flex items-center gap-2 text-sm font-bold text-[#8a2be2]">
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
        {/* Daily Quiz */}
        <div className="neon-rgb-border rounded-2xl mb-8 bg-[var(--color-panel)]/60 backdrop-blur-sm">
          <div className="p-6 sm:p-8">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
              <div className="neon-rgb-border rounded-xl h-14 w-14 shrink-0 flex items-center justify-center bg-[var(--color-panel)]/60 backdrop-blur-sm">
                <span className="text-2xl">🎯</span>
              </div>
              ...
              <Link href="/quiz/daily" className="neon-rgb-border rounded-xl shrink-0 bg-[var(--color-panel)]/60 backdrop-blur-sm px-5 py-2.5 flex items-center gap-1.5 text-[13px] font-bold text-[#ffd700]">
                Play Daily
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-8">
          {RANKS.map((rank) => (
            <div key={rank.name} className="neon-rgb-border rounded-xl bg-[var(--color-panel)]/60 backdrop-blur-sm p-3 text-center">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-8">
          {CATEGORIES.map((cat) => (
            <Link key={cat.name} href={`/quiz/play?category=${cat.name}`} className="neon-rgb-border rounded-xl h-full block bg-[var(--color-panel)]/60 backdrop-blur-sm p-4 hover:scale-[1.02] transition-transform">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            ...
            <div key={item.title} className="neon-rgb-border rounded-xl bg-[var(--color-panel)]/60 backdrop-blur-sm p-4">
