...
export async function GET(req: NextRequest) {
  ...
  const where: Record<string, unknown> = { userId: session.user.id, subType: "light_novel" };
...
export async function POST(req: NextRequest) {
  ...
  const entry = await prisma.mangaEntry.upsert({
    ...
    update: {
      title, coverImage, subType: "light_novel", status: status || "PLANNING",
    ...
    create: {
      userId: session.user.id, mediaId, title, coverImage, subType: "light_novel",
  ...
  if (status === "COMPLETED") {
    const count = await prisma.mangaEntry.count({
      where: { userId: session.user.id, status: "COMPLETED", subType: "light_novel" },
    });
    if (count >= 5) checkAndAwardAchievement(session.user.id, "LN_ENTHUSIAST").catch(() => {});
    const totalCompleted = await prisma.mangaEntry.count({
      where: { userId: session.user.id, status: "COMPLETED" },
    });
    if (totalCompleted >= 5) checkAndAwardAchievement(session.user.id, "BOOKWORM").catch(() => {});
  }
