...
export default function HorizontalScroll({
  ...
  return (
    <section className="relative overflow-hidden border-b border-[var(--color-line)]">
      {/* Netflix-style gradient wash background */}
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 py-10 sm:py-14">
        <div className="mb-6 flex items-end justify-between">
          <div>
            {eyebrow && (
              <p className="font-mono text-[10px] uppercase tracking-[0.25em] text-[var(--color-cyan)]">
                {eyebrow}
              </p>
            )}
            <h2 className="font-display text-3xl font-black sm:text-4xl md:text-5xl tracking-tight">
              {title}
            </h2>
            {subtitle && (
              <p className="mt-1 text-sm text-[var(--color-mute)]">{subtitle}</p>
            )}
          </div>
          {viewAllHref && (
            <Link
              href={viewAllHref}
              className="shrink-0 text-xs uppercase tracking-wider text-[var(--color-mute)] hover:text-white transition-colors"
            >
              View all
            </Link>
          )}
      ...
      <div className="relative group">
            ...
            >
              ...
              <div className="relative z-10 ml-[30px]">
                <div className="relative aspect-[2/3] overflow-hidden rounded-[8px] bg-[#161616] transition-all duration-500 group-hover/card:scale-[1.02] group-hover/card:shadow-[0_0_40px_-8px_var(--color-magenta)]">
                  ...
                  {/* Minimal overlay with just title + score */}
        ...
        {canScrollLeft && (
          <button
            onClick={() => scroll("left")}
            className="absolute left-1 top-[calc(50%-20px)] z-20 hidden sm:flex h-10 w-10 items-center justify-center rounded-[8px] bg-black/90 text-white border border-white/10 hover:border-white/30 transition-all hover:scale-105"
          >
        ...
        {canScrollRight && (
          <button
            ...
            className="absolute right-1 top-[calc(50%-20px)] z-20 hidden sm:flex h-10 w-10 items-center justify-center rounded-[8px] bg-black/90 text-white border border-white/10 hover:border-white/30 transition-all hover:scale-105"
          >
