...
export default function ShareButton({ mediaId, title, episode }: { mediaId: number; title: string; episode?: number }) {
  ...
  const discord = () => {
    window.open(`https://discord.com/share?text=${encodeURIComponent(`${text} ${shareUrl}`)}`, "_blank", "width=600,height=400");
  };

  return (
    <div className="flex items-center gap-2">
      <button onClick={tweet} title="Share on Twitter/X"
        className="p-2 rounded-lg bg-white/5 hover:bg-white/10 transition-colors">
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
      </button>
      <button onClick={discord} title="Share on Discord"
        className="p-2 rounded-lg bg-white/5 hover:bg-white/10 transition-colors">
